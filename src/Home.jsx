...
export default function App() {
  ...
  useEffect(() => {
    ...
    const lenis = new Lenis({
      ...
      smooth: true,
      smoothTouch: false, // leave touch native for now
    });
        ...
        <div className="w-full max-w-[1200px] mx-auto px-6 md:px-8 flex items-center justify-between h-full">
          ...
          <div className="hidden md:flex flex-initial items-center justify-center gap-10 nav-fade h-full">
            {['How It Works', 'Pricing', 'About Us', 'Contact'].map(link => {
              ...
              return (
                <a key={link} href={href} className="text-xs font-bold tracking-[2px] text-white/70 uppercase hover:text-brand-green transition-colors duration-300">{link}</a>
              );
            })}
          </div>
          ...
          <div className="md:col-span-3 flex flex-col items-start text-left">
            ...
            <div className="flex flex-col gap-4">
              {[
                ...
                <a key={i} href={link.href} className="text-sm font-medium hover:text-white transition-colors duration-200">
                  {link.label}
                </a>
              ))}
            </div>
